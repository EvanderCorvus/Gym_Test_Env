...
def NNSequential(dimensions, activation, output_activation=nn.Identity, batch_norm=False):
    layers = []
    for i in range(len(dimensions)-1):
        act = activation if i < len(dimensions)-2 else output_activation
        if batch_norm and i < len(dimensions)-2:
            layers += [nn.Linear(dimensions[i], dimensions[i+1]),
                       nn.BatchNorm1d(dimensions[i+1]),
                       act()]
        else: layers += [nn.Linear(dimensions[i], dimensions[i+1]),
                         act()]
